    ...
    <ul class="pagination">
        ...
        <?php if ($arResult["NavPageNomer"] > 1): ?>
            <?php if ($arResult["NavPageNomer"] > 2): ?>
                <li>
                    <a
 href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult
 ["NavNum"]?>=<?=($arResult["NavPageNomer"]-1)?>" aria-label="Previous">
                        <span aria-hidden="true"><?=GetMessage("nav_prev")
 ?></span>
                    </a>
                </li>
            <?php else: ?>
...
?></span>
 ...
 $arResult["bSavePage"] == false) : // если ссылка на первую страницу ?>
                <li><a
 href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><?=$arResult
 ["nStartPage"]?></a></li>
            <?php else: ?>
                <li><a
 href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult
 ["NavNum"]?>=<?=$arResult["nStartPage"]?>"><?=$arResult["nStartPage"]?></a></li>
            <?php endif; ?>
 ...
 aria-label="NEXT">
                    <span aria-hidden="true"><?=GetMessage("nav_next")
 ?></span>
                </a>
            </li>
        <?php endif; ?>
